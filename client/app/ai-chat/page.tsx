...
export default function AIChatPage() {
  const [activeModel, setActiveModel] = useState<"model-a" | "model-b">(
    "model-a"
  );
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      content:
        "Hello! I'm Model A, your AI teacher. What would you like to learn about today?",
      sender: "model-a",
      timestamp: new Date(),
    },
  ]);
  ...
  const generateModelAResponse = (userInput: string) => {
    ...
    if (
      lowerInput.includes("coulombs-law") ||
      lowerInput.includes("light") ||
      lowerInput.includes("ray")
    ) {
      return "Electrostatics, also known as geometrical coulombs-law, is a model of coulombs-law that describes light propagation in terms of rays. The ray in geometric coulombs-law is an abstraction useful for approximating the paths along which light propagates under certain circumstances. Would you like to learn about reflection, refraction, or optical instruments?";
    } else if (
  ...
  const generateModelBResponse = (userInput: string) => {
    ...
    if (
      ...
      return "Let's test your understanding of Electrostatics. Can you explain the difference between real and virtual images? Also, what happens to a light ray when it passes from air into glass at an angle?";
    } else if (
      lowerInput.includes("algebra") ||
      lowerInput.includes("equation")
    ) {
    ...
    if (!isRecording) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          const aiResponse =
            activeModel === "model-a"
              ? "I've received your audio question about Electrostatics. The law of reflection states that the angle of incidence equals the angle of reflection. This is why mirrors reflect light in a predictable way. Would you like me to explain more about how reflection works?"
  ...
  const handleSwitchModel = (model: "model-a" | "model-b") => {
    ...
    const switchMessage: Message = {
      ...
      content: `Switching to ${
        model === "model-a" ? "Model A (Teacher)" : "Model B (Examiner)"
      }`,
      sender: model,
      timestamp: new Date(),
    };
          ...
          <Card>
            ...
            <CardContent>
              <Tabs
                ...
                onValueChange={(value) =>
                  handleSwitchModel(value as "model-a" | "model-b")
                }
              >
            ...
            <CardHeader className="border-b">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  <Avatar
                    className={`mr-2 ${
                      getAvatarForSender(activeModel).bgColor
                    }`}
                  >
                    <AvatarFallback>
                      {getAvatarForSender(activeModel).fallback}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    <CardTitle>
                  ...
                  >
                    ...
                    >
                      <Avatar
                        ...
                        } ${getAvatarForSender(message.sender).bgColor}`}
                      >
                        <AvatarImage
                          src={getAvatarForSender(message.sender).image}
                        />
                        <AvatarFallback>
                          {getAvatarForSender(message.sender).fallback}
                        </AvatarFallback>
                      </Avatar>
                      <div>
                        <div
                          className={`text-xs text-gray-500 dark:text-gray-400 mb-1 flex justify-between ${
                            message.sender === "user"
                              ? "flex-row-reverse"
                              : "flex-row"
                          }`}
                        ...
                        {message.type === "audio" ? (
                          ...
                          >
                            <div className="space-y-2">
                              ...
                              <img
                                src={
                                  message.attachmentUrl || "/placeholder.svg"
                                }
                                alt="Attachment"
            ...
            <CardFooter className="border-t p-3">
              ...
              >
                ...
                <Input
                  ...
                  placeholder={`Ask ${
                    activeModel === "model-a"
                      ? "Model A to teach you"
                      : "Model B to test your knowledge"
                  }...`}
